...
- Use `go mod tidy` to clean up unused dependencies (run before commits)
...
- Use the `WaitGroup.Go` method (Go 1.25+):
  ```go
...
- Prefer the enhanced `net/http` `ServeMux` with pattern-based routing and method matching (Go 1.22+)
